...
export const RangeRangePicker: FC<RangeDatePickerType> = ({
	setRange,
	setFilter,
	filtered,
}) => {
	...
	return (
		<>
			<Space id="range-space" className="w-100 justify-content-between">
				...
				<ConfigProvider
					direction={langOption("rtl", "ltr")}
					locale={langOption(arEG, enUS)}
				>
					<DatePicker
						id="range-picker"
						onChange={(e) => setRange([e, e?.add(30, "day")])}
						disabledDate={disabledDate}
						format="DD/MM/YYYY"
						placeholder={langOption("تاريخ البدء", "Start Date")}
					/>
				</ConfigProvider>
			</Space>
			<Form.Check
