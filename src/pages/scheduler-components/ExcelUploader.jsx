...
import { cacheFiles } from "./excel-components/CacheFiles";
...
const ExcelUploader = ({
  ...
  const { lang, finalWorkbook, msgMaker, deleteMessage } = useContext(ContextData);
  return (
    <>
      <div id="file-uploader" className="bg-body-secondary border rounded mx-auto position-relative">
        <input
          ...
          multiple
          accept=".xls, .xlsx"
          onChange={(e) => cacheFiles(lang, uploaded, setUploaded, msgMaker, deleteMessage)(e)}
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-primary">
            {lang === "AR" ? "اضغط لرفع الكشوفات" : "Click to upload your files"}
            <br />
      ...
      <div>
        <ButtonGroup dir="ltr" className="excel-btn-gp d-block mx-auto">
          <Button className="w-75" onClick={handleUpload}>
            {lang === "AR" ? "رفع الملفات" : "Submit"}
            {"	"}({uploaded.length})
          </Button>
        ...
        <Button
          ...
          disabled={finalWorkbook.length === 0}
          onClick={previewWorkbook}
        >
          {lang === "AR" ? "عرض البيانات المرفوعة" : "Preview"}
        </Button>
      </div>
    </>
  );
