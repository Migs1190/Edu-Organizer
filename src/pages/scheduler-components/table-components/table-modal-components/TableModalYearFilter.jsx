...
export const TableModalYearFilter = ({ timeTable, searchPeriod, limiter, langOption, rowCounter }) => (
  <>
    {timeTable.map((T, Ti) => {
      if (searchPeriod(T.Period1, limiter, "year") || searchPeriod(T.Period2, limiter, "year")) {
        return (
          ...
          <Fragment key={Ti}>
            ...
            {T.Period1.map(
              (subject, tpi) =>
                subject.enrolledStudents.some((student) => student.year === limiter) && (
                  // biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
                  <tr key={tpi}>
